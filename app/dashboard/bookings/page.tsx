...
export default function BookingsPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        const [bookingsData, professionalsData] = await Promise.all([
          api.bookings.list(token),
          api.professionals.list(token),
        ]);

        setBookings(bookingsData.data || bookingsData || []);
        setTotal(bookingsData.total || (bookingsData.data || bookingsData || []).length || 0);
  ...
  async function assignProfessional(bookingId: string, professionalId: string) {
    ...
    try {
      ...
      console.error('Error assigning professional:', e);
    } finally {
      setAssigning(null);
    }
  }
  ...
  if (selected) {
    const pros = professionals.filter(
      (p: any) => (p.isAvailable ?? p.is_available ?? false) === true
    );
          ...
          <div>
            ...
            <div className="space-y-3">
              {pros.length === 0 ? (
                ...
                pros.map((pro: any) => {
                  const fullName = pro.fullName || pro.full_name || 'Profesional';
                  const serviceRadius =
                    pro.serviceRadiusMiles ?? pro.service_radius_miles ?? '-';
                  const avgRating = Number(pro.avgRating ?? pro.avg_rating ?? 0).toFixed(1);
                  const totalServices = pro.totalServices ?? pro.total_services ?? 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-400">
                          Radio: {serviceRadius}mi · Rating: {avgRating} · {totalServices}{' '}
                          servicios
                        </p>
                      </div>

                      <button
                        onClick={() => assignProfessional(selected.id, pro.id)}
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              {[
                ...
                <button
                  ...
                  className={
                    'w-full text-left px-4 py-3 text-sm border-b border-gray-50 hover:bg-gray-50 flex items-center justify-between ' +
                    (selected.status === status ? 'bg-emerald-50' : '')
                  }
                >
                  <span>{status.replace(/_/g, ' ')}</span>
                  {selected.status === status && (
                    <span className="text-emerald-600 text-xs">● Actual</span>
                  )}
                </button>
              ))}
            </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-medium text-gray-900">Bookings ({total})</h1>
      </div>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={
                        ...
                        (SC[b.status] || 'bg-gray-50 text-gray-600')
                      }
                    >
