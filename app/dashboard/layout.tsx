...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const links = [
    { href: '/dashboard', label: t(locale, 'summary') },
    { href: '/dashboard/leads', label: t(locale, 'leads') },
    { href: '/dashboard/bookings', label: t(locale, 'bookings') },
    { href: '/dashboard/professionals', label: t(locale, 'professionals') },
  ];
  return (
    <div className='min-h-screen bg-gray-50 flex'>
      <aside className='w-52 bg-white border-r border-gray-200 flex flex-col'>
        ...
        <div className='p-3 border-t border-gray-200'>
          <button onClick={logout} className='w-full text-left px-3 py-2 text-sm text-gray-500 hover:text-gray-700 rounded-lg hover:bg-gray-50'>{t(locale, 'closeSession')}</button>
        </div>
      </aside>
